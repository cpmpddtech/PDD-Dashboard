/**
 * ═══════════════════════════════════════════════════════════════
 *  PDD Dashboard — Cloudflare Worker v2.7.0
 ...
 *    CF_WORKER_SCRIPT_NAME → Worker script name (e.g. pdd-dashboard)
 *    GOOGLE_API_KEY        → Google Sheets API key (read)
 *    SPREADSHEET_ID        → Google Sheet ID
 *    FINANCE_SPREADSHEET_ID→ Google Finance Sheet ID
 *    GMAIL_CLIENT_ID       → OAuth2 client ID (from Google Cloud Console)
 *    GMAIL_CLIENT_SECRET   → OAuth2 client secret
 *    GMAIL_REFRESH_TOKEN   → OAuth2 refresh token
 *    DRIVE_REFRESH_TOKEN   → OAuth2 refresh token with drive.file + spreadsheets + gmail.send scopes
 *    DRIVE_FOLDER_ID       → Google Drive folder ID root
 *    DRIVE_JSON_FOLDER_ID  → Google Drive subfolder for Reports_JSON
 *    DRIVE_PDF_FOLDER_ID   → Google Drive subfolder for Reports_PDF
 *    ALERT_EMAIL_TO        → email address to send alerts to
 *    ALERT_THRESHOLD       → number e.g. 0.70 for 70% threshold
 *    DASHBOARD_URL         → live dashboard URL for email footer links
 */
...
async function driveWriteFile(token, folderId, filename, content, existingFileId = null) {
  ...
  const body = [
    ...
    `--${boundary}`, 'Content-Type: application/json', '', JSON.stringify(content),
    `--${boundary}--`,
  ].join('\r\n');
  ...
  return `
    <div style="font-family:sans-serif;max-width:560px;margin:0 auto;">
      <div style="background:#1e3a5f;padding:24px 28px;border-radius:12px 12px 0 0;">
        <div style="color:#fff;font-size:20px;font-weight:800;">⚠️ PDD Performance Alert</div>
      ...
      <div style="background:#fff;border:1px solid #e5e7eb;border-top:none;padding:24px 28px;border-radius:0 0 12px 12px;">
        ...
        <p style="color:#6b7280;font-size:12px;margin-top:16px;">This is an automated alert from the PDD Dashboard.</p>
      </div>
    </div>`;
}
...
async function checkAndSendAlert(env, weeklyRows) {
  ...
  if (!thisWeek || thisWeek.t === 0) return;
  ...
  if (thisWeek.p < threshold) {
    try {
      ...
      await sendEmail(token, env.ALERT_EMAIL_TO, `⚠️ PDD Alert: CW ${currentCW} performance at ${Math.round(thisWeek.p*100)}%`, html);
    } catch(e) {
...
function buildNotificationEmail(event, data, env) {
  const BASE_URL = env.DASHBOARD_URL || 'https://cpmpddtech.github.io/PDD-Dashboard/';
    ...
    try {
          ...
          (async () => {
            try {
              ...
              const row = [
                ...
                resolvedUsername, u.name, u.role, ip
              ];
      ...
      if (action === 'add_program') {
        ...
        const projRow = [
          ...
          program.startQuarter, program.endQuarter, duration, 0
        ];
      ...
      if (action === 'add_task') {
        ...
        const row = [
          taskId, task.programId, task.programName, task.name,
          task.quarter, cwNum, task.status || 'Planned', task.owner, task.projectName || '',
        ];
      ...
      // ══════════════════════════════════════════════
      //  NEW: send_announcement (admin only)
      //  Sends a fully custom HTML email to one or
      //  more recipients. Used for launch emails and
      //  any one-time bulk announcements from console.
      // ══════════════════════════════════════════════
      if (action === 'send_announcement') {
        ...
        const gmailRefreshToken = env.GMAIL_REFRESH_TOKEN || env.DRIVE_REFRESH_TOKEN;
        if (!env.GMAIL_CLIENT_ID || !env.GMAIL_CLIENT_SECRET || !gmailRefreshToken)
          return new Response(JSON.stringify({ ok: false, error: 'Gmail OAuth not configured' }), { status: 200, headers });

        try {
          const token = await getAccessToken(env.GMAIL_CLIENT_ID, env.GMAIL_CLIENT_SECRET, gmailRefreshToken);

          // Support single address (string) or array of addresses
          const recipients = Array.isArray(to) ? to : [to];
          const results = await Promise.allSettled(
            recipients.map(addr => sendEmail(token, addr, subject, html))
          );
      ...
      // ── Save notification opt-out prefs ───────────
      ...
      if (action === 'save_roles') {
        ...
        try {
          ...
          return new Response(JSON.stringify({
            ok: false, manualPaste: true, config, error: e.message,
      ...
      if (action === 'setup_sheets') {
        ...
        const sheetsToSetup = [
          { name: 'Main Programs',  headers: ['Program ID','Program Name','Start CW','End CW','Start Quarter','End Quarter','Duration Weeks','Target Tasks'] },
          { name: 'Weekly Reports', headers: ['Date','CW','Name','Program','Done','Missed','Blockers','Notes','Time'] },
          { name: 'Login Audit',    headers: ['Date','Time','Username','Name','Role','IP'] },
        ];
        ...
        for (const sheet of sheetsToSetup) {
          ...
          const hasCorrectHeader = existing.some(row => row.some(cell => String(cell).trim() === firstExpected));

          if (hasCorrectHeader) { skipped.push(sheet.name); continue; }
          ...
          if (existing.length === 0) {
            ...
            if (!addRes.ok) {
              ...
              if (!e.error?.message?.includes('already exists')) continue;
            }
          }
        ...
        for (const tsName of taskSheets) {
          ...
          if (colAH_filled === 0 && colJQ_filled) {
            const dataRows = tsRows.slice(1).filter(row => row[9] !== null && row[9] !== undefined && row[9] !== '');
            if (dataRows.length === 0) continue;
            const cleanRows = dataRows.map(r => [r[9]||'', r[10]||'', r[11]||'', r[12]||'', r[13]||'', r[14]||'', r[15]||'', r[16]||'']);
            await fetch(
              `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${encodeURIComponent(tsName)}:clear`,
              { method: 'POST', headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' } }
            );
            await appendToSheet(token, spreadsheetId, tsName, ['Task ID','Program ID','Program Name','Task Name','Quarter','CW','Status','Owner']);
            for (const row of cleanRows) { await appendToSheet(token, spreadsheetId, tsName, row); }
      ...
      if (action === 'get_finance_data') {
        ...
        try {
          ...
          const [budgetRes, txRes, queueRes, propRes] = await Promise.all([
            finFetch('Budget'), finFetch('Transactions'), finFetch('Approval_Queue'), finFetch('Budget_Proposals'),
          ]);
      ...
      if (action === 'log_expense') {
        ...
        try {
          const token = await getDriveToken(env);
          const txRow = [txId, now.slice(0,10), program, projectId || '', category, amount, description, user.name || user._key, 'pending', '', ''];
          await fetch(
            `https://sheets.googleapis.com/v4/spreadsheets/${finSheetId}/values/Transactions:append?valueInputOption=USER_ENTERED`,
            { method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ values: [txRow] }) }
          );
          ...
          await fetch(
            ...
            { method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ values: [queueRow] }) }
          );
      ...
      if (action === 'approve_expense' || action === 'reject_expense') {
        ...
        try {
          ...
          const txData = await (await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${finSheetId}/values/Transactions`, { headers: { Authorization: `Bearer ${token}` } })).json();
          const txRows = txData.values || [];
          const txRowIdx = txRows.findIndex((r, i) => i > 0 && r[0] === txId);
          if (txRowIdx > 0) {
            ...
            await fetch(
              ...
              { method: 'PUT', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ values: [[newStatus, user.name || user._key, now]] }) }
            );
          }

          const qData = await (await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${finSheetId}/values/Approval_Queue`, { headers: { Authorization: `Bearer ${token}` } })).json();
          ...
          if (qRowIdx > 0) {
            ...
            await fetch(
              ...
              { method: 'PUT', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ values: [[newStatus, user.name || user._key, now, notes || '']] }) }
            );
          }
          ...
          if (action === 'approve_expense') {
            try {
              ...
              const txProgram = txRow[2] || '', txprojectId = txRow[3] || '', txCategory = txRow[4] || '';
              const allTxData = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${finSheetId}/values/Transactions`, { headers: { Authorization: `Bearer ${token}` } }).then(r => r.json());
              const allTxRows = allTxData.values || [];
              const totalSpent = allTxRows.slice(1)
                .filter(r => r[2] === txProgram && (r[3] || '') === txprojectId && r[4] === txCategory && (r[8] === 'approved' || r[0] === txId))
                .reduce((sum, r) => sum + parseFloat(r[5] || 0), 0);
              const bData = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${finSheetId}/values/Budget`, { headers: { Authorization: `Bearer ${token}` } }).then(r => r.json());
              const bRows = bData.values || [];
              const bRowIdx = bRows.findIndex((r, i) => i > 0 && r[1] === txProgram && (r[2] || '') === txprojectId && r[3] === txCategory);
              if (bRowIdx > 0) {
                const allocated = parseFloat(bRows[bRowIdx][4] || 0);
                const remaining = allocated - totalSpent;
                await fetch(
                  `https://sheets.googleapis.com/v4/spreadsheets/${finSheetId}/values/Budget!F${bRowIdx+1}:G${bRowIdx+1}?valueInputOption=USER_ENTERED`,
                  { method: 'PUT', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ values: [[totalSpent, remaining]] }) }
                );
              }
            } catch(_) {}
          }
          ...
          if (txRowIdx > 0) {
            ...
            await sendNotification(env, 'expense_reviewed', {
              txId, description: txRow[6] || '', amount: txRow[5] || 0,
              status: newStatus, reviewedBy: user.name || body.username, submittedBy: txRow[7] || '',
            }).catch(() => {});
          }
      ...
      if (action === 'update_budget') {
        const user = validateUser(body.username, body.password);
        if (!user) return new Response(JSON.stringify({ error: 'Unauthorized' }), { status: 401, headers });
        if (user.role !== 'admin') return new Response(JSON.stringify({ error: 'Admin only' }), { status: 403, headers });

        const { budgetId, allocated, notes } = body;
        if (!budgetId || !allocated) return new Response(JSON.stringify({ error: 'Missing budgetId or allocated' }), { status: 400, headers });

        const finSheetId = env.FINANCE_SPREADSHEET_ID;
        try {
          const token = await getDriveToken(env);
          const bData = await (await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${finSheetId}/values/Budget`, { headers: { Authorization: `Bearer ${token}` } })).json();
          const bRows = bData.values || [];
          const bRowIdx = bRows.findIndex((r, i) => i > 0 && r[0] === budgetId);
          if (bRowIdx < 1) return new Response(JSON.stringify({ error: 'Budget ID not found' }), { status: 404, headers });
          ...
          await fetch(
            ...
            { method: 'PUT', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
              body: JSON.stringify({ values: [[allocated, bRows[bRowIdx][5] || 0, parseFloat(allocated) - parseFloat(bRows[bRowIdx][5] || 0), bRows[bRowIdx][7] || '', notes || bRows[bRowIdx][8] || '']] }) }
          );
          return new Response(JSON.stringify({ ok: true }), { status: 200, headers });
        } catch (e) {
          return new Response(JSON.stringify({ ok: false, error: e.message }), { status: 500, headers });
      ...
      if (action === 'add_budget') {
        ...
        if (!program || !category || !allocated) return new Response(JSON.stringify({ error: 'Missing required fields: program, category, allocated' }), { status: 400, headers });
        ...
        try {
          ...
          const spent = 0, remaining = parseFloat(allocated) - spent;
      ...
      if (action === 'get_projects') {
        ...
        try {
          ...
          const projects = data.map(r => Object.fromEntries(hdrs.map((h,i) => [h.trim(), r[i] || '']))).filter(r => r['Project ID']);
      ...
      if (action === 'add_project') {
        ...
        if (!programId || !projectName) return new Response(JSON.stringify({ error: 'Missing programId or projectName' }), { status: 400, headers });
        ...
        try {
          ...
          const now = new Date().toISOString();
          await appendToSheet(token, env.SPREADSHEET_ID, 'Projects',
            [projectId, programId, projectName, description || '', startCW || '', endCW || '', quarter || '', status || 'Planning', responsible || '', user.name || user._key, now]);
          await sendNotification(env, 'project_created', {
            projectId, projectName, programId, programName: programId, createdBy: user.name || body.username,
          }).catch(() => {});
      ...
      if (action === 'get_proposals') {
        ...
        try {
          ...
          let proposals = data.map(r => Object.fromEntries(hdrs.map((h,i) => [h.trim(), r[i] || '']))).filter(r => r['Proposal ID']);
          if (user.role === 'finance_staff') proposals = proposals.filter(p => p['Proposed By'] === (user.name || user._key));
        ...
        try {
          ...
          if (!hasCorrectHeader) {
            ...
            if (rows.length === 0) {
              ...
              await fetch(
                ...
                { method: 'PUT', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ values: [PROP_HEADERS] }) }
              );
            }
          ...
          if (proposalId) {
            ...
            await fetch(
              `https://sheets.googleapis.com/v4/spreadsheets/${finSheetId}/values/${encodeURIComponent('Budget_Proposals')}!A${sheetRow}:N${sheetRow}?valueInputOption=USER_ENTERED`,
              {
                ...
                body: JSON.stringify({ values: [[
                  proposalId, programId, projectId, category || '', requestedAmount, justification,
                  period || '', notes || '', proposedBy,
                  existingRow[9] || now, existingRow[10] || 'draft',
                  existingRow[11] || '', existingRow[12] || '', existingRow[13] || ''
                ]] })
              }
            );
            ...
            await appendToSheet(token, finSheetId, 'Budget_Proposals',
              [newId, programId, projectId, category || '', requestedAmount, justification, period || '', notes || '', proposedBy, now, 'draft', '', '', '']);
      ...
      if (action === 'submit_proposal') {
        ...
        try {
          ...
          await fetch(
            ...
            { method: 'PUT', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ values: [['submitted']] }) }
          );

          const submittedRow = rows[rowIdx];
          await sendNotification(env, 'proposal_submitted', {
            proposalId, programId: submittedRow[1] || '', category: submittedRow[3] || '',
            amount: submittedRow[4] || 0, proposedBy: submittedRow[8] || user.name,
          }).catch(() => {});
      ...
      if (action === 'review_proposal') {
        ...
        try {
          ...
          await sendNotification(env, 'proposal_reviewed', {
            proposalId, category: row[3] || '', status: decision,
            reviewedBy: user.name || body.username, reviewNotes: reviewNotes || '', proposedBy: row[8] || '',
